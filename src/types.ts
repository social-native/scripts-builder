...
export type SetArgsObject = (
  input: IState
) => { argsObj: { [arg: string]: string } } & IState;
export type IArgsObj = ReturnType<SetArgsObject>;

// setDefaultConfigPath: defaultPath -> input -> input + defaultConfigPath
export type SetDefaultConfigPath = (config: {
  defaultPath: string;
}) => (input: IState) => { defaultConfigPath: string } & IState;
...
export type SetUserConfigPath = (config: {
  optionNames: string[];
}) => (
  input: IState & IArgsObj
) => { userSpecifiedConfigPath: string } & IState;
...
export type ModifyRelativePathsInConfigObject = (
  config: IShouldModifyPath & IFieldsWithModifiablePaths & IState
) => (input: ConfigObj & OriginDir) => ConfigObj & IState;
...
interface IFieldsUpdater {
  fieldsUpdater: (
    input: IState & OriginDir & ConfigObj
  ) => { [fieldPath: string]: any };
}
export type AddFieldsToConfigObject = (
  config: IFieldsUpdater
) => (input: IState & ConfigObj & OriginDir) => ConfigObj & IState;

// writeConfigObjectToPath: tempConfigPath -> input + configObj -> input + tempConfigFilePath
export type WriteConfigObjectToPath = (config: {
  tempConfigFilePath: string;
}) => (input: IState & ConfigObj) => { tempConfigFilePath: string } & IState;
...
export type GenerateCommand = (
  input: IState & TempConfigFilePath & ConfigObj & ArgsArr
) => { command: string } & IState;
export type Command = ReturnType<GenerateCommand>;

// executeCommand -> input -> input + commandStatus
export type ExecuteCommand = (
  input: IState & Command
) => { commandStatus?: { message?: string; code?: number } } & IState;
export type CommandResponse = ReturnType<ExecuteCommand>;

export type SetSpecifiedOriginDir = (
  input: IState & IArgsObj
) => IArgsObj & OriginDir & IState;
